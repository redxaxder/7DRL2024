...
static func random_bonus_skill(skill_name: String, rng_seed: int, column: int = 0) -> Skill:
	var skill = Skill.new()
	skill.kind = Skill.Kind.Bonus
	skill.name = skill_name
	skill.bonuses = Skill.random_bonus(rng_seed, column)
	return skill
	...
	for row in 3:
		for column in 7:
			if (row + column) % 2 == 0:
				...
				append_skill(
					random_bonus_skill(SkillName.generate_name(), rng.randi(), column), row
					)
